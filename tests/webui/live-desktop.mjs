...
if (!runtime || !process.env.SYNCSHELL_CORE) throw new Error('Set SYNCSHELL_TEST_RUNTIME and SYNCSHELL_CORE');
await stat(join(runtime, '.syncshell-port-fixture'));
const configPath = join(runtime, 'home/config.xml'), xml = await readFile(configPath, 'utf8');
...
async function api(route, method = 'GET', body) {
    const response = await fetch(base + '/rest/' + route, {method,
...
await writeFile(join(temporary, 'xdg-open'), `#!/usr/bin/env node\nconst fs=require('node:fs'), cp=require('node:child_process');\nif(process.argv[2].startsWith('file:'))fs.writeFileSync(${JSON.stringify(launchFile)},process.argv[2],{mode:0o600});\nelse process.exit(cp.spawnSync('/usr/bin/xdg-open',process.argv.slice(2)).status ?? 1);\n`, {mode:0o700});
const core = spawn(resolve(process.env.SYNCSHELL_CORE), ['stream', '--host-id', 'omarchy', '--config', configPath], {
    env:{...process.env, PATH:temporary + ':' + process.env.PATH}, stdio:['pipe','pipe','pipe']});
let frames = '', diagnostics = '';
core.stdout.on('data', data => {frames += data;}); core.stderr.on('data', data => {diagnostics += data;});
async function until(check) {for (let i=0;i<200;i++) {if(await check())return; await new Promise(r=>setTimeout(r,100));} throw new Error('Timed out');}
const browser = await chromium.launch({headless:true, executablePath:process.env.SYNCSHELL_CHROMIUM || '/usr/bin/chromium'});
const page = await browser.newPage({viewport:{width:1908,height:954}});
let folder, directory;
try {
    await until(()=>frames.includes('"online":true'));
    core.stdin.write(JSON.stringify({v:1,type:'action',id:'desktop-open',action:'webui.open',args:{}})+'\n');
    await until(async()=>{try{return Boolean(await readFile(launchFile,'utf8'));}catch{return false;}});
    ...
    const source=join(directory,filename), target=join(directory,'notes.txt');
    ...
    await row.getByRole('button',{name:'Recheck files in folder',exact:true}).click();
    await until(()=>auto.isEnabled());
    for (const control of [row.getByRole('button',{name:'Open folder',exact:true}), row.locator('a.review-file')]) {
    ...
    await page.getByRole('dialog').getByText('the original name already exists; no file was replaced').waitFor();
    assert.match(await readFile(target,'utf8'),/^An original/);
    assert.match(await readFile(source,'utf8'),/^Keep/);
    await rm(target);
    await page.getByRole('button',{name:'Rename',exact:true}).click();
    await until(()=>row.count().then(n=>n===0));
    ...
    await writeFile(join(evidence,'desktop-results.json'),JSON.stringify({result:'passed',checks:[
        'production core launch through private bootstrap', 'direct Syncthing serving without review injection',
        'default containing-folder opening from both controls', 'destination appearing during dialog is preserved',
        'permission denial disables rename early without preventing folder opening',
        'confirmed rename retains bytes and clears conflict row', 'tab reload retains local connection',
    ]},null,2)+'\n');
    ...
    await new Promise(resolve => {
        ...
        core.once('exit',()=>{clearTimeout(timer);resolve();}); core.stdin.end();
    });
    if(directory) {await chmod(directory,0o700).catch(()=>{}); await rm(directory,{recursive:true,force:true}); await api('db/scan?folder='+folder.id,'POST').catch(()=>{});}
