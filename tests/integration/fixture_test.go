...
func TestFixtureConfigIsolatesGeneratedDefaults(t *testing.T) {
	data := []byte(`<configuration version="37"><folder id="default" path="/unwanted"/><device id="self"/><gui tls="false"><address>127.0.0.1:8384</address><apikey>fixture-key</apikey></gui><options><listenAddress>default</listenAddress><globalAnnounceEnabled>true</globalAnnounceEnabled><localAnnounceEnabled>true</localAnnounceEnabled><relaysEnabled>true</relaysEnabled><natEnabled>true</natEnabled><urAccepted>0</urAccepted><unrelated>retain</unrelated></options></configuration>`)
	result, err := fixtureConfig(data, "127.0.0.1:18401", "tcp://127.0.0.1:18411")
	if err != nil {
		t.Fatal(err)
	}
	var config struct {
	...
	if len(config.Folders) != 0 || config.Device.ID != "self" || config.GUI.Address != "127.0.0.1:18401" || config.GUI.Key != "fixture-key" || config.Options.Unrelated != "retain" {
