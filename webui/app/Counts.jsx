...
export function Counts({info, prefix = 'global'}) {
    ...
    const full = <>
        <div><span><span class="far fa-fw fa-copy" /> {t('Files')}:</span><span>{(files || 0).toLocaleString()}</span></div>
        <div><span><span class="far fa-fw fa-folder" /> {t('Folders')}:</span><span>{(folders || 0).toLocaleString()}</span></div>
        <div><span><span class="far fa-fw fa-hdd" /> {t('Total')}:</span><span>~{unitPrefixed(bytes, true)}B</span></div>
    </>;
    return <>
