...
export function ActionDialog({action, state, api, session, onClose}) {
    ...
    if (action.type === 'listeners' || action.type === 'discovery') {
        ...
        return <Dialog title={action.type === 'listeners' ? health.failed.length ? 'Listener Failures' : 'Listener Status' : health.failed.length ? 'Discovery Failures' : 'Discovery Status'} status={health.failed.length ? 'danger' : 'default'} icon="fas fa-sitemap" onClose={onClose}>
            {action.type === 'listeners' ? <p>{t(health.running ? 'Syncthing is listening on the following network addresses for connection attempts from other devices:' : 'Syncthing is not listening for connection attempts from other devices on any address.  Only outgoing connections from this device may work.')}</p> : <><p>{t(health.running ? 'The following methods are used to discover other devices on the network and announce this device to be found by others:' : 'This device cannot automatically discover other devices or announce its own address to be found by others.  Only devices with statically configured addresses can connect.')}</p><p>{t('Failure to connect to IPv6 servers is expected if there is no IPv6 connectivity.')}</p></>}
