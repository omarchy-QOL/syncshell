            ...
            <form ref={form} onSubmit={event => { event.preventDefault(); save(); }}>
                ...
                {advanced ? <>
                    ...
                    {tab === 'Ignored Devices' ? <>
                        ...
                        {tab === 'General' && <>
                            <label for="settings-api-key">{t('API Key')}</label><div class="input-group"><input id="settings-api-key" class="form-control" type="text" readOnly value={draft.gui.apiKey} /><span class="input-group-btn"><button type="button" class="btn btn-default" onClick={generateKey}>{t('Generate')}</button></span></div>
