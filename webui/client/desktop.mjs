...
export function desktopActions(browser = window) {
    ...
    const fileRequest = (device, group, file = group.current || group.copies[0]) => ({
        device,
        folder: group.folder,
        root: group.root,
        path: group.path,
    ...
    return {
        ...
        check: (device, group, file) => request('check', fileRequest(device, group, file)),
        open: (group, file, device) => request('open', fileRequest(device, group, file)),
        rename: (group, file, device) => request('rename', fileRequest(device, group, file))
    };
}
